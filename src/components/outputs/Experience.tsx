          ...
          return (
            <div className="company-group" key={company.company}>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#565f89] text-xs">·</span>
                <span className="text-[#565f89] text-xs">{company.totalPeriod}</span>
              </div>

              {/* Roles under this company */}
              <div className="ml-6 mt-2 space-y-4 border-l border-[#565f89]/30 pl-4">
                {company.roles.map((role, roleIndex) => (
                  <div className="role-entry" key={role.title}>
                    ...
                    <div className="ml-6 mt-0.5 text-[#565f89] text-xs">{role.period}</div>
                    <div className="ml-6 mt-2 space-y-1">
                      {role.responsibilities.map((resp) => (
                        <div className="flex items-start gap-2" key={resp}>
                          <span className="text-[#565f89] mt-0.5">│</span>
                          <span className="text-[#c0caf5] text-sm">{resp}</span>
                        </div>
                      ))}
                      <div className="flex flex-wrap gap-2 mt-2 pl-3 border-l border-[#565f89]/20">
                        {role.tech.map((t) => {
                          const TechIcon = ICONS[t.icon]
                          return (
                            <div
                              className="flex items-center gap-1 bg-[#1a1b26] border border-[#565f89]/40 rounded px-1.5 py-0.5"
                              key={t.name}
                            >
